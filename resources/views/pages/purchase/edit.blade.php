...
{{-- @csrf kepake untuk token ,wajib --}}
            ...
            <form method="POST" action="{{ route("purchase-edit", $purchase->id ) }}">
             {{-- @csrf kepake untuk token ,wajib --}}
                @csrf
                {{-- <div class="mt-3">
                ...
                <div class="mt-3">
                    ...
                    onblur="(this.type='text')" placeholder="Purchase_deadine"  value = "{{ old("purchase_deadline", $purchase->purchase_deadline) }}">
                    @error("purchase_deadline")
                    <p style = "color: red; font-size: 10px;">{{$message }}</p>
                    @enderror
                </div>
                <div class="mt-3">
                    <input type="text" class="form-control" name="register" placeholder="Register"  value = "{{ old("register", $purchase->register) }}">
                    @error("register")
                    <p style = "color: red; font-size: 10px;">{{$message }}</p>
                    @enderror
                </div>
                <div class="mt-3">
                    ...
                    onblur="(this.type='text')" placeholder="Purchase Date" value = "{{ old("purchase_date", $purchase->purchase_date)}}">
                    @error("purchase_date")
                    <p style = "color: red; font-size: 10px;">{{$message }}</p>
                    @enderror
                </div>
