...
{{-- @csrf kepake untuk token ,wajib --}}
            @csrf
            <input type="text" name="product_name" placeholder="Nama Barang" value = "{{ old("product_name" ) }}">
                @error("product_name")
                <p style = "color: red; font-size: 10px;">{{$message }}</p>
                @enderror

                <input type="text" name="price" placeholder="Harga" value = "{{ old("price") }}">
                @error("price")
                <p style = "color: red; font-size: 10px;">{{$message }}</p>
                @enderror
                ...
                <input type="text" name="variant" placeholder="Variant"  value = "{{ old("variant") }}">
                @error("variant")
                <p style = "color: red; font-size: 10px;">{{$message }}</p>
                @enderror
