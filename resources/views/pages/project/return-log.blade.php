...
@section('content')
    <x-container>
        <div class="w-100 d-flex align-items-center justify-content-between">
            <h3 class="mt-4">Daftar Pengembalian Barang</h3>
        </div>
                ...
                @forelse ($return_items as $ri)
                    @foreach($ri->return_item_products as $rip)
                        ...
                        <tr>
                            ...
                            {{-- <td class="border border-1 border-secondary ">{{ number_format($subtotal, 0, ',', '.') }}<br><i style="font-weight: 600;">(Price: {{ $rip->product->price }}, disc: {{ $rip->product->discount }}%, qty: {{ $dop->quantity }})</i></td> --}}
