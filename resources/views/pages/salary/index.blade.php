                    ...
                    @foreach($attendances->groupBy('project_id') as $proj_id => $gbp)
                        ...
                        @if($total_jam_normal != 0)
                            <tr class="detail-area{{ $emp_id }}" style="display: none; background-color: @if($iterasus % 2 == 1) #E0E0E0 @else white @endif;">
                                ...
                                <td class="border border-1 border-secondary" class="py-2" colspan="4">Normal: {{ $total_jam_normal }} hari ({{ $project_name }})</td>
                    ...
                    @else
                        @foreach($prepays as $ppay)
                            @if($total_gaji - $ppay->cut_amount > 0)
                                <tr class="detail-area{{ $emp_id }}" style="display: none; background-color: @if($iterasus % 2 == 1) #E0E0E0 @else white @endif;">
                                    <td class="border border-1 border-secondary" class="py-2"></td>
                                    <td class="border border-1 border-secondary" class="py-2" colspan="4">Potongan kasbon untuk {{ $ppay->remark }} (Sisa saldo: {{ number_format(($ppay->curr_amount - $ppay->cut_amount < 0 ? 0 : $ppay->curr_amount - $ppay->cut_amount), 0, ',', '.') }})</td>
