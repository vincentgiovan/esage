...
{{-- @csrf kepake untuk token ,wajib --}}
            ...
            <form method="POST" action="{{ route("product-edit",$product->id ) }}">
                {{-- @csrf kepake untuk token ,wajib --}}
                            @csrf
                            <div class="mt-3">
                                <input type="text" class="form-control" name="product_name" placeholder="Nama Barang" value = "{{ old("product_name", $product->product_name  ) }}">
                                @error("product_name")
                                <p style = "color: red; font-size: 10px;">{{$message }}</p>
                                @enderror
                            </div>
                            <div class="mt-3">
                                <input type="text" class="form-control" name="unit" placeholder="Unit"  value = "{{ old("unit", $product->unit) }}">
                                @error("unit")
                                <p style = "color: red; font-size: 10px;">{{$message }}</p>
                                @enderror
                            </div>
                            <div class="mt-3">
                                <select name="status" class="form-select">
                                    @foreach ($status as $s)
                                        <option value="{{ $s }}" @if ($product->status == $s ) selected @endif>{{ $s }}</option>
                                    @endforeach
