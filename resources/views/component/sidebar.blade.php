<nav class="fixed-top h-100" id="sidebar" style="background-color: rgb(69, 69, 69); z-index: 10;">
    <ul class="nav d-flex flex-column items-stretch flex-nowrap gap-2 py-3 overflow-y-auto" id="sidebar-menu">
        ...
        @if(Gate::allows('user-role', ['master', 'accounting_admin', 'gudang', 'subgudang']))
            <li class="nav-item">
        ...
        @if(Gate::allows('user-role', ['master', 'accounting_admin']))
            @php
                $lastSaturday = Carbon\Carbon::now()->previous(Carbon\Carbon::SATURDAY);
                $thisWeekFriday = $lastSaturday->copy()->addDays(6);
            @endphp
            <li class="nav-item">
                <a class="nav-link text-decoration-none px-4" style="color: white; font-weight: bold; @if (Request::is("salary*")) background-color: green; @else rgb(69, 69, 69); @endif" href="{{ route("salary-index", ['from' => $lastSaturday->toDateString(), 'until' => $thisWeekFriday->toDateString()]) }}" ><i class="bi bi-currency-dollar me-2"></i> Gaji Pegawai</a>
            </li>
        @endif
