<nav class="fixed-top h-100" id="sidebar" style="background-color: rgb(69, 69, 69); z-index: 10;">
    <ul class="nav d-flex flex-column items-stretch flex-nowrap gap-2 py-3 overflow-y-auto" id="sidebar-menu">
        ...
        @if(in_array(Auth::user()->role, ['master', 'accounting_admin']))
            <li class="nav-item">
                <a class="nav-link text-decoration-none px-4" style="color: white; font-weight: bold; @if (Request::is("*employee*leaves*")) background-color: green; @else rgb(69, 69, 69); @endif" href="{{ route("leave-admin-index") }}" ><i class="bi bi-building-fill-x me-2"></i> Pengajuan Cuti</a>
            </li>
        @endif
    </ul>
</nav>
